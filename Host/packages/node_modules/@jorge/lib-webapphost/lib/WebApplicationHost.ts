...
export class WebApplicationHost implements IWebApplicationHost {
  ...
  addRoute(routeProvider: IRouteProvider): void {
    var expressRouteProvider = routeProvider as IExpressRouteProvider;

    if (expressRouteProvider != null) {
      this.app.use(routeProvider.path, expressRouteProvider.getExpressRouter());
    }
  }
  ...
  public constructor(
    ...
    this.app.use(function(
      ...
      Reflect.defineMetadata(
        "container",
        new ServiceProvider(childContainer),
        req
      );
  ...
  public initialize(): void {
    ...
    let routeProviders: IRouteProvider[] = this.serviceProvider.getServices<
      IRouteProvider
    >(TRouteProvider);

    routeProviders.forEach(e => {
      let erp: IExpressRouteProvider = e as IExpressRouteProvider;
      if (erp != null) {
        var router = erp.getExpressRouter();
        this.app.use(erp.path, router);
      }
    });
